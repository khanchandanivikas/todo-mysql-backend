...
const recuperarCompleteTodos = (req, res, next) => {
  const sql = "SELECT * FROM todo WHERE complete = 'true' ";
  connection.query(sql, (error, results) => {
    if (error) throw error;
    if (results.length > 0) {
      res.json(results);
...
const recuperarIncompleteTodos = (req, res, next) => {
  const sql = "SELECT * FROM todo WHERE complete = 'false' ";
  connection.query(sql, (error, results) => {
    if (error) throw error;
    if (results.length > 0) {
      res.json(results);
...
const editTodo = (req, res, next) => {
  ...
  const sql = `UPDATE todo SET task = '${task}' WHERE id = ${idTodo}`;
...
const deleteTodo = (req, res, next) => {
  ...
  const sql = `DELETE FROM todo WHERE id = ${idTodo}`;
...
const deleteCompletedTodo = (req, res, next) => {
  const sql = "DELETE FROM todo WHERE complete = 'true' ";
